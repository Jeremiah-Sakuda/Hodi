...
| ≥1 Google Agent Framework | **Antigravity SDK** (four agents + supervisor); **ADK dual-listed as the fallback**, decided Aug 8 | HOD-020 |
| ≥1 Google Cloud infra service | Cloud Run services + Jobs, Firestore, Cloud Scheduler, Cloud Logging/Trace | HOD-410 |
| New project in Submission Period | Public remote from first commit, unsquashed; pre-Aug-3 audit; **`## Relationship to my other submissions`** disclosing the shared claim-record lineage by path and date | HOD-001, HOD-004 |
| Category | The Fortified Enterprise Fleet | — |
| **Fleet: cataloged for cross-department use** | **Agent Registry** — agents published with versions, scopes, and owning function; discoverable and invocable by role | HOD-330 |
| **Fleet: context across weeks of async operation** | **Memory Bank** — grants, scopes, and revocations are long-lived state with temporal validity; the registry survives instance death; crawler logs accumulate for weeks | HOD-320, HOD-331 |
| **Fleet: production data without violating policy** | **Agent Identity** (per-agent scoped SAs), **Agent Gateway** (routing + policy enforcement at the boundary), **Model Armor** (untrusted buyer/scope documents) | HOD-311, HOD-312, HOD-313 |
...
| **Nexus: failure tolerance** | Supervisor quarantine + reroute on loop, timeout, or scope violation; per-agent circuit breaker; `TaskAbandoned` on deadline | HOD-341 |
...
| Devpost — other data sources | `docs/devpost-description.md` §3 — **the corpus is the author's own published work** | HOD-622 |
| **Devpost — findings and learnings** | `docs/devpost-description.md` §4, **drafted Aug 12–13**, `{{fill from metrics.json}}` slot | HOD-622, HOD-509 |
| ≤4-min video, public YouTube | §6 shot list; target 3:40, hard cap 4:00 | HOD-601–608 |
...
| Hosted project | Public registry + `/.well-known/hodi.json` + signed buyer API; `min-instances=0`, max capped | HOD-411 |
...
| **Build-toolchain evidence is reachable** | README `## Technologies used` links `docs/BUILD-LOG.md` and `docs/antigravity/decision.md`; Antigravity labelled on Diagram A; blog leads its second half with the Antigravity multi-agent + OTel findings | HOD-510, HOD-505, HOD-621 |
...
- **HOD-104** **Scope lattice — structure.** Use-type partial order (`training ⊃ fine_tuning ⊃ rag_retrieval`; `human_reference` contains nothing; `synthesis` incomparable to `human_reference`), declared as data in `src/schema/lattice.py`, not as branching logic. *AC (property: the order is a checkable artifact, not scattered conditionals):* the relation is exported as a table and `make verify-scopes` prints it.
- **HOD-106** **Scope lattice — containment resolution.** `permits(grant_scope, requested_scope) -> bool` resolving across **all five dimensions simultaneously**: use-type containment, model class, commercial status, territory, and temporal validity. *AC (property: containment is real, not string matching):* a truth table of ≥40 cases in CI, including — a `training` grant answers "may I fine-tune" **yes**; a `human_reference` grant answers "may I train" **no**; a commercial-permitted grant answers a non-commercial request **yes** (containment runs one way only); a territory-limited grant answers an out-of-territory request **no**; an expired grant answers everything **no**; two overlapping grants from the same counterparty resolve to the **union**, not the first match.
...
- **HOD-601–608** Video, §6.
- **HOD-620–624** Bonus stack, §7. **Blog and Devpost description drafted Aug 12–13**, published Aug 28; Devpost draft submitted **Aug 27**; submit **Aug 30**. The blog's first half is the honesty architecture (why there is no training-membership class); **its second half is `## Building a governed fleet on Antigravity`** — headless multi-agent execution under distinct service accounts, OTel span emission carrying agent identity, and the Aug 8 boolean and its outcome. That is genuinely new information for the team that owns the SDK, and it is the highest-value thing this project can tell a Google panel.
