...
BEGIN_MARKER = "<!-- GENERATED by scripts/deployment_status.py from docs/deployment_status.json."


def readme_table_bounds(readme: str):
    """(start, end) of the embedded generated table, or None if it is absent."""
    start = readme.find(BEGIN_MARKER)
    if start == -1:
        return None
    # The table ends at the paragraph that follows the last row.
    tail = readme.find("\n\n", readme.find("`○ scripted, never run`", start))
    return None if tail == -1 else (start, tail)


def embedded_table(readme: str) -> str:
    bounds = readme_table_bounds(readme)
    return "" if bounds is None else readme[bounds[0]:bounds[1]]


def write_readme(doc: dict) -> bool:
    """
    Regenerate the table embedded in README.md. Returns True if it changed.

    WHY THIS IS A COMMAND AND NOT A MANUAL EDIT. The table is marked GENERATED
    and says not to hand-edit it, but nothing regenerated it and nothing compared
    it — so `record_live_verification.py` could promote a capability in the JSON
    and leave the README still showing the old state and the old date. The
    generated artifact must be reproducible from its source by a command, or
    "generated" is just a comment.
    """
    readme = README_PATH.read_text()
    bounds = readme_table_bounds(readme)
    if bounds is None:
        raise SystemExit("README.md no longer contains the generated deployment table "
                         "markers — refusing to guess where it belongs.")
    fresh = render(doc)
    if readme[bounds[0]:bounds[1]] == fresh:
        return False
    README_PATH.write_text(readme[:bounds[0]] + fresh + readme[bounds[1]:])
    return True
...
def main() -> int:
    ...
    ap.add_argument("--write-readme", action="store_true",
                    help="regenerate the table embedded in README.md from the JSON")
    args = ap.parse_args()
    ...
    if args.write_readme:
        changed = write_readme(doc)
        print("README deployment table regenerated." if changed
              else "README deployment table already current.")
        return 0
