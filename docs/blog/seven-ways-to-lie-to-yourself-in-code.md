...
Hodi's first empirical headline was a negative result: *I published machine-readable consent terms at a discoverable endpoint and no crawler asked.* It was an honest reading of the metric at the time. It was also wrong: a later audit found a self-identifying crawler request that the detector's word-boundary bug had missed.
...
Reporting those ten as crawler access would have been exactly the fabricated finding the project exists to refuse, arrived at from the opposite direction. So the metric changed shape: `known_crawler_ua_matches` is the only number the project describes as crawler access, while every other non-self request stays *unattributed*. The later detector correction found matching user agents after all — **29 visits** as of the 2026-08-29 audit, from three distinct self-identifying crawlers. Twenty-one of them fetched `/robots.txt`. Exactly one — ever — fetched the machine-readable consent document linked from it. That is the current finding. It is still attribution, not authentication—a user agent is whatever the requester chose to send—and the live count remains sourced from `docs/metrics.json`, not from this essay.
