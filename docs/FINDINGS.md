...
**Serverless Gemma (HOD-303, HOD-005):** `gemma-4-26b-a4b-it-maas` @ global returns HTTP 200 (observed classifying a vendor-prefixed crawler user agent → `bot`). Serverless per-token Gemma makes the fenced GPU project design unnecessary — the triage tier now calls Vertex Gemma with Ollama and heuristic fallbacks, and classified a live record (`human`) inside the deployed accrual audit.
...
**It was wrong, by one regex anchor.** The crawler signatures were generic on purpose — `\bbot\b`, `bot/`, `[-_]bot`, `bot[-_]` — to avoid naming vendors and to catch crawlers nobody has heard of. But `\bbot\b` requires a word boundary *before* `bot`, and the commonest crawler-naming convention in existence is a vendor prefix glued straight onto it. A user agent of the form `<Vendor>Bot` — no separator, no boundary — matches none of the four patterns, and that shape covers most of the major crawlers by name. The detector was blind to the single most likely shape of the thing it was built to detect.

**What was actually in the log.** One record, `2026-08-11`: a self-identifying, vendor-prefixed crawler user agent, path `/robots.txt`, `robots_txt_fetched_first: true`. A self-identifying crawler arrived, read the robots file — and **did not fetch `/.well-known/hodi.json`**, which is one request away and is where the machine-readable terms are served.
...
## 2026-08-29 — The negative result finished turning positive, one request at a time

**The consent document has now been fetched — once.** The 2026-08-29 audit reads 6956 accrued records; 29 match a crawler signature, from three distinct self-identifying user agents between 2026-08-11 and 2026-08-29. Twenty-one of the 29 fetched `/robots.txt`. Exactly one — ever — fetched `/.well-known/hodi.json`, the machine-readable terms named in that same `robots.txt`. The project's signature phrasing ("every one fetched robots.txt; not one fetched the terms") was therefore retired from every document that carried it: both halves stopped being true in the same audit. This is the third time this finding has changed shape under measurement (zero → a detector bug → 17-and-none → 29-and-one), and each time the dated audit is what allowed the prose to move without any prior claim having been a lie when made.
