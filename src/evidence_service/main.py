...
@app.get("/metrics-snapshot", response_class=JSONResponse)
async def get_metrics_snapshot(request: Request):
    """
    The latest COMMITTED crawler-accrual audit (HOD-790), served so that no
    live page ever hard-codes an audit figure again.

    The guided demo's first screen shipped with the crawler count as a literal
    in its HTML — 17 — and the next audit moved the number to 29 while the live
    page kept saying 17. That is the Literal Metric Rendering Rule violated on
    the project's most-viewed surface (defect `demo-page-hardcoded-crawler-count`).
    This route serves the same `docs/metrics.json` the README and Diagram B are
    held to, with its audit date attached, so every surface cites ONE dated
    source. It is a committed observation, not a live count — the date is the
    honesty. Unreadable file → 503, never a plausible number.
    """
    try:
        metrics_path = os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(
            os.path.abspath(__file__)))), "docs", "metrics.json")
        with open(metrics_path, encoding="utf-8") as fh:
            accrual = json.load(fh)["daily_crawler_accrual_metrics"]
    except Exception as e:  # noqa: BLE001
        logger.error(f"metrics snapshot unavailable: {e}")
        return JSONResponse(status_code=503, content={"status": "unavailable"})
    return {
        "source": "docs/metrics.json — committed audit, regenerated by `make metrics`",
        "audit_date_utc": accrual.get("audit_date_utc"),
        "known_crawler_ua_matches": accrual.get("known_crawler_ua_matches"),
        "known_crawler_distinct_user_agents": len(accrual.get("known_crawler_user_agents") or []),
        "known_crawler_robots_txt_fetches": (accrual.get("known_crawler_paths") or {}).get("/robots.txt"),
        "known_crawler_consent_doc_fetches": accrual.get("known_crawler_consent_doc_fetches"),
        "known_crawler_first_seen": accrual.get("known_crawler_first_seen"),
        "known_crawler_last_seen": accrual.get("known_crawler_last_seen"),
        "total_accrued_records": accrual.get("total_accrued_records"),
        "claim_limit": ("A user-agent match is attribution, not authentication. "
                        "known_crawler_ua_matches is the only figure this project "
                        "describes as crawler access."),
    }
