...
- **Training-set membership is not determinable, and Hodi cannot say it.** Membership inference against frontier models is an unsolved problem. The guarantee is **structural**: the `EvidenceRecord.class` enum has no such value, so the schema cannot express the claim as data, and every record carries the literal `"This record does not assert training-set membership."` A render-time lint additionally rejects an enumerated list of overclaim phrases in free text — but it is a backstop, not a paraphrase detector, and we measure it rather than assert it: against a probe set of 12 paraphrases it was deliberately not written against, **it rejects 4** (`overclaim_lint_coverage` in [docs/metrics.json](docs/metrics.json), regenerated by `make lint-coverage`). The schema is the invariant; the lint reduces the chance of a bad sentence, and we would rather publish its real hit rate than imply it catches everything.
- **Revocation terminates a grant. It does not un-train a model.** Revocation is a legal instrument with technical enforcement of the grant, a dated notice, and a receipt (whose `signature` field is a labelled placeholder — see below). A lint asserts that no generated notice implies removal from a model.
- **`crawler_access` is designed and instrumented, but no crawler access has been observed.** The evidence endpoint has logged every access since Aug 6, 2026. As of the 2026-08-08 audit ([docs/metrics.json](docs/metrics.json), regenerated by `make metrics`): **539 accrued records, of which 0 match any known AI-crawler or search-crawler user agent.** 517 are this project's own instrumented tooling. The remaining 22 are non-self-originated but browser-like and are reported as *unattributed*, not as crawler access — 9 of them arrived in a single one-second burst from cloud IPs and included a request to `/api/v1/debug/compromised_agent_read`, which is inspection traffic, not crawling. `known_crawler_ua_matches` is the only figure this project will call crawler access, and it is zero. *"I published machine-readable consent terms at a discoverable endpoint and no crawler asked"* is itself an instrumented finding about current scraper etiquette. The endpoint keeps accruing; `make check-docs` fails the build if these numbers drift from `metrics.json`.
...
- **The four service accounts are policy identities, not four runtime principals — but the runtime identity is genuinely constrained.** Two separate guarantees, kept separate here because conflating them is how the claim went wrong once. (1) *Conflict-of-interest separation* — which agent may read whose data — is enforced in-process by the policy gateway, using the same `iam_policy.py` data the generated matrix renders; the deployed service is a **single Cloud Run process**, not four services each with `--service-account`, so at runtime that separation is application-layer, not GCP IAM. Splitting into four services is the stated next step. (2) *Append-only* — that grant history cannot be rewritten — **is** enforced by IAM at runtime: the process executes as a dedicated runtime identity (`hodi-runtime-sa`) holding the append-only custom role plus read-only Firestore access, with **no `datastore.entities.update` or `.delete`** in its effective permissions. This was not always true — the service previously ran as the default compute account with `roles/editor`, which can update and delete; the fix (a create-only runtime SA, and gateway writes changed from `.set()` upsert to `.create()`) is in [scripts/deploy_gcp.sh](scripts/deploy_gcp.sh), and [`tests/test_grant_log_iam.py`](tests/test_grant_log_iam.py) reads the deployed identity's roles back from IAM and asserts the union excludes update and delete. See [docs/FINDINGS.md](docs/FINDINGS.md).
...
- **The `signature` field on notices, receipts and grant events is a labelled placeholder — not a cryptographic signature.** Every value is produced by [`src/schema/signing.py`](src/schema/signing.py) and reads `UNSIGNED_PLACEHOLDER:<kind>:<id>`, so a reader dumping a receipt sees what it is worth; nothing in the codebase verifies one, because there is nothing to verify. This is **not** fixed by adding HMAC: a shared secret makes a notice verifiable only by parties who could equally forge it, which over a legal artifact is worse than an honest placeholder. Real signing needs an asymmetric key the recipient can check without being able to mint — Cloud KMS or a managed Ed25519 key, plus distribution, rotation and a verification endpoint. That is a feature, not a rename, and it has not been built. [`tests/test_signature_honesty.py`](tests/test_signature_honesty.py) fails if any code path emits a value that looks like a real signature.
...
- **[Seven ways to lie to yourself in code](https://jeremiah-sakuda.github.io/Hodi/blog/seven-ways-to-lie-to-yourself-in-code.html)** *(published)* — the defect ledger as a write-up: twenty-seven defects, nine classes, the three that recurred, the meta-pattern behind all of them, and the four structural guards that answer it. Source: [docs/blog/](docs/blog/seven-ways-to-lie-to-yourself-in-code.md).
...
- **Blog — [Seven ways to lie to yourself in code](https://jeremiah-sakuda.github.io/Hodi/blog/seven-ways-to-lie-to-yourself-in-code.html)**. The defect ledger: twenty-seven defects across nine classes, the three that recurred, the Antigravity SDK assertion that was verified before it was made, and the four structural guards. Created for the All Things Agentic Hackathon.
