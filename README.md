...
- **Cloud Run** — the deployed evidence endpoint and buyer API (services), and the headless verification harness plus nightly teardown (jobs). `min-instances=0`, max capped. The revocation worker runs as its own private service under the propagator SA ([scripts/deploy_revocation_worker.sh](scripts/deploy_revocation_worker.sh), **executed 2026-08-14**, revision `hodi-revocation-worker-00004-m7t`; the deploy proves its identity, its append-and-read-only effective permissions, and that an anonymous call gets 403 while an authenticated one gets 200) — real workload identity, and the killable isolation the in-process daemon thread cannot provide. `/api/v1/revoke` still runs the cascade in-process rather than calling that worker; the worker is deployed and proven, and the caller has not been cut over.
