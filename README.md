...
- **The four service accounts are policy identities, not four runtime principals.** The conflict boundary is real and enforced — by an in-process policy gateway that every read passes through, using the same `iam_policy.py` data the generated matrix renders. But the deployed service is a **single Cloud Run process running as one identity** (`406699565497-compute@developer.gserviceaccount.com`), not four services each deployed with `--service-account`. The four SAs exist in GCP with the append-only custom role ([scripts/deploy_gcp.sh](scripts/deploy_gcp.sh)), and they are the identities the policy layer names, checks, and writes into every denial event and OTel span — but at runtime the separation is application-layer, not GCP IAM. Splitting the fleet into four separately-deployed services is the natural next step and is stated here as not-yet-done rather than implied.
