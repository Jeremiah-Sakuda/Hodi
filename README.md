...
- **A `signature` field is either verifiable or says it proves nothing — and which one you get depends on the deployment, so read the field.** Three states, never conflated, all produced by [`src/schema/signing.py`](src/schema/signing.py) and reported live at [`/verification-key`](https://hodi-evidence-endpoint-406699565497.us-central1.run.app/verification-key):
  1. `KMS-ECDSA-P256-SHA256:<key>:<sig>` — real asymmetric signing, private key never leaving Cloud KMS, verifiable by anyone holding the public key and mintable by no one else. **Provisioned by [scripts/setup_kms_signing.sh](scripts/setup_kms_signing.sh), which has NOT yet been run against the live project** — so the deployed service is not emitting these today. `docs/deployment_status.json` is the machine-readable source for that, and `make check-docs` fails if this paragraph disagrees with it.
  ...
  3. `UNSIGNED_PLACEHOLDER:<kind>:<id>` — what every path emits when no signer is configured, which **includes the currently deployed service**. It is a labelled non-signature: nothing verifies it, because there is nothing to verify.
...
| `conflict_domain_separation` | ▣ in-process only | src/gateway/gateway.py + tests/test_gateway_session_scope.py (offline); make demo-live (deployed, over the network) | 2026-08-12T16:45:00Z |
| `kms_signing` | ○ scripted, never run | scripts/setup_kms_signing.sh (never run); tests/test_signing.py covers the code path offline | — |
