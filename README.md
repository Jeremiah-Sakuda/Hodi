...
- **[Seven ways to lie to yourself in code](https://jeremiah-sakuda.github.io/Hodi/blog/seven-ways-to-lie-to-yourself-in-code.html)** *(published)* — the defect ledger as a write-up: forty-five defects, nine classes, the four that recurred, the meta-pattern behind all of them, and the four structural guards that answer it. Source: [docs/blog/](docs/blog/seven-ways-to-lie-to-yourself-in-code.md).
...
- **Cloud Run** — the deployed evidence endpoint and buyer API (services), four private conflict-domain workloads, and the headless verification harness plus nightly teardown (jobs). `min-instances=0`, max capped. A private revocation worker was last observed under the propagator SA on 2026-08-14. The repository now hardens its database IAM and routes `/api/v1/revoke` to it with no fail-open local fallback, but that post-review cutover is **not yet observed live**; deployment status remains the authority.
...
- **Blog — [Seven ways to lie to yourself in code](https://jeremiah-sakuda.github.io/Hodi/blog/seven-ways-to-lie-to-yourself-in-code.html)**. The defect ledger: forty-five defects across nine classes, the four that recurred, the Antigravity SDK assertion that was verified before it was made, and the four structural guards. Created for the All Things Agentic Hackathon.
