...
- **`crawler_access` is instrumented, and the crawlers came — and none of them asked.** The evidence endpoint has logged every access since Aug 6, 2026. As of the **2026-08-21** audit ([docs/metrics.json](docs/metrics.json), regenerated by `make metrics`): **3291 accrued records, of which 9 match a crawler user-agent signature.** Those 9 are the finding, and it sharpened as they accumulated: **two distinct self-identifying AI crawlers, nine visits between 2026-08-11 and 2026-08-20, and every single one fetched `/robots.txt` — while not one fetched [`/.well-known/hodi.json`](https://hodi-evidence-endpoint-406699565497.us-central1.run.app/.well-known/hodi.json), one request away, named in that very `robots.txt`, where the machine-readable consent terms live.** 3217 records are this project's own instrumented tooling. The remaining 74 are non-self-originated but browser-like and are reported as *unattributed*, not as crawler access. `known_crawler_ua_matches` is the only figure this project will call crawler access, and the vendors are not named in prose — they did nothing wrong, and the point is not who they are. **This number was 0 until 2026-08-12, and it was wrong:** the detector's pattern required a word boundary *before* `bot`, so the commonest crawler-naming convention — a vendor prefix glued straight onto `bot` — never matched. The claim is narrower and more interesting than the null result it replaced: *the terms are published, discoverable, and reachable in one request from the file every crawler did read — and over ten days, none asked.* `make check-docs` fails the build if these numbers drift from `metrics.json`.
