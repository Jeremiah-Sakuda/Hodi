...
- **Revocation reaches narrower grants, not broader ones.** Revoking use type R terminates every active grant held at R or at anything R contains — revoking `training` reaches `fine_tuning`, `rag_retrieval` and `human_reference`, walked from the lattice's covering relation. It does **not** narrow a grant held *above* R. Revoke `fine_tuning` while a counterparty holds `training`, and the training grant stands and still permits fine-tuning: 6 of the 25 (held × revoked) pairs behave this way, pinned by [`tests/test_revocation_reach.py`](tests/test_revocation_reach.py). This is a modelling limit, not a missing branch — `Scope.use_type` holds one value and the use types form a chain, so "training but not fine_tuning" is not expressible, and the only way to stop the narrower use would be to terminate the broader grant outright, destroying a permission the artist did not revoke and cannot restore under an append-only log. Doing that silently would be the worse failure. See [docs/FINDINGS.md](docs/FINDINGS.md) for the full matrix and the design options.
