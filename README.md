...
- **[The Market](https://hodi-evidence-endpoint-406699565497.us-central1.run.app/#market)** *(buyers)* — pick a work and ask for a licence **in your own words**. The pinned Gemini interpreter reads the sentence live on Vertex AI; the artist's declared terms and the append-only log decide, deterministically. A grant is a real event; a refusal lists its reasons; a revocation also closes the standing offer, so "I take it back" cannot be re-granted by asking again.
...
- **`crawler_access` is instrumented, and the crawlers came — and none of them asked.** The evidence endpoint has logged every access since Aug 6, 2026. As of the **2026-08-29** audit ([docs/metrics.json](docs/metrics.json), regenerated by `make metrics`): **6956 accrued records, of which 29 match a crawler user-agent signature.** Those 29 are the finding, and the finding moved as they accumulated: **three distinct crawler-signature user agents, 29 visits between 2026-08-11 and 2026-08-29. Twenty-one of them fetched `/robots.txt` — and exactly one, ever, fetched [`/.well-known/hodi.json`](https://hodi-evidence-endpoint-406699565497.us-central1.run.app/.well-known/hodi.json), one request away, named in that very `robots.txt`, where the machine-readable consent terms live.** For the first eighteen days that consent-fetch count was zero; the earlier phrasing "not one fetched the terms" is retired because it stopped being true, which is how a dated observation is supposed to age. 6675 records are this project's own instrumented tooling. The remaining 281 are non-self-originated but browser-like and are reported as *unattributed*, not as crawler access. `known_crawler_ua_matches` is the only figure this project will call crawler access, and the vendors are not named in prose — they did nothing wrong, and the point is not who they are. **This number was 0 until 2026-08-12, and it was wrong:** the detector's pattern required a word boundary *before* `bot`, so the commonest crawler-naming convention — a vendor prefix glued straight onto `bot` — never matched. The claim is narrower and more interesting than the null result it replaced: *the terms are published, discoverable, and reachable in one request from the file the crawlers did read — and in nineteen days they have been read once.* `make check-docs` fails the build if these numbers drift from `metrics.json`.
...
- **The agent identities are enforced at three altitudes, with unverified cutover work labelled.** (1) *Append-only* — grant history cannot be rewritten — is runtime IAM: the front door executes as a create-only identity (`hodi-runtime-sa`, no `datastore.entities.update`/`.delete`), verified live by [`tests/test_grant_log_iam.py`](tests/test_grant_log_iam.py). (2) *The domain boundary is a credential boundary* — executed 2026-08-14: identity, commercial, evidence, and adjudication collections were migrated into four named Firestore databases, each served by a private Cloud Run workload under its own IAM-conditioned service account. The front door is refused by Google IAM on all four and must invoke the owning workload. The shared append-only grant log remains in `(default)` by design, so counterparty row separation inside it remains gateway-enforced. (3) *Revocation-worker hardening and front-door cutover — verified live 2026-08-25*: [`scripts/deploy_revocation_worker.sh`](scripts/deploy_revocation_worker.sh) replaces the propagator's broad viewer and append grants with `(default)`-only conditions, and `/api/v1/revoke` executes through that private workload with no fail-open local fallback. Measured through the deployed path on rev `00059-z55`: median 2389 ms warm. [`docs/deployment_status.json`](docs/deployment_status.json) remains the authority and now records **12 verified capabilities and zero unproven**.
...
- **The red-team drill: six attacks, one command** — `make red-team`. An injected "grant unlimited rights" document (flagged, lattice unmoved); a compromised negotiator reaching for artist identity (denied by IAM policy, not an `if`); **role spoofing — presenting one agent's identity while claiming another's role, refused because the gateway binds role to service account and never reads it from the caller**; the evidence agent asserting model-training (refused by the schema *and* the authority matrix); a rogue worker committing after quarantine (its execution lease revoked, its write refused); a tampered incident package (verification fails, then verifies again once restored). Every boundary that yields exits nonzero, so the drill runs in CI, and the legitimate transaction still completes at the end.
...
- **[Seven ways to lie to yourself in code](https://jeremiah-sakuda.github.io/Hodi/blog/seven-ways-to-lie-to-yourself-in-code.html)** *(published)* — the defect ledger as a write-up: sixty-six defects, nine classes, the four that recurred, the meta-pattern behind all of them, and the four structural guards that answer it. Source: [docs/blog/](docs/blog/seven-ways-to-lie-to-yourself-in-code.md).
...
- **OpenTelemetry → Cloud Trace** — every agent decision emits a span carrying `agent.identity`, `policy.consulted`, and `outcome`, nested inside ADK's own `invoke_agent` spans, so a whole delegation reads as a single trace **in a durable backend, not only on stdout**. Retrievable by id: `POST /api/v1/fleet/delegation_drill` returns the `trace_id` it wrote and the exporter that wrote it (`cloud_trace` or `console` — the response never implies a durable trace exists when it does not). Verified 2026-08-14 on trace `35f6bc26c177a22e99d7d491ead3b6b1`: 11 spans, `invocation` → `invoke_agent fleet_orchestrator` → negotiator `PERMITTED`, `registry.discover` **`NOT_DISCLOSED`** (the buyer's negotiator is not told the propagator exists), custodian `INITIATED`, `registry.discover` `DISCOVERED`, propagator `ABANDONED`, supervisor `QUARANTINED_AND_REROUTED`.
...
- **Firestore** — the append-only grant-event log. Deterministic event IDs, `create()`-only discipline, custom IAM role withholding `update`/`delete` from every agent SA. State is always a fold over events. The durable registry and Memory Bank fold over the same store; per-domain named databases are the workload-identity-separation target ([scripts/setup_workload_identity.sh](scripts/setup_workload_identity.sh), **executed 2026-08-14**, with the conditioned IAM bindings proven live by `HODI_E2E=1 tests.test_workload_identity` via service-account impersonation). The domain collections were migrated out of the default database; `conflict_domain_separation` in the table below reads `✓ verified`. What remains gateway-enforced is counterparty ROW separation *inside* the shared append-only grant log, which stays in `(default)` by design because every domain-appropriate identity must reach it — a boundary that cannot be solved by moving databases.
...
- **Cloud Run** — the deployed evidence endpoint and buyer API (services), four private conflict-domain workloads, and the headless verification harness plus nightly teardown (jobs). `min-instances=0`, max capped. A private revocation worker runs under the propagator SA with `(default)`-only conditioned grants, and `/api/v1/revoke` executes through it with no fail-open local fallback — **verified live 2026-08-25** against rev `00059-z55`; deployment status remains the authority.
...
- **First commit:** `9d3e8cece39fdb3c39859ab776f001e6257936ed` at `2026-08-06T12:37:20-05:00` (`2026-08-06T17:37:20Z`). **The published history was rewritten on 2026-08-25**, which changed every commit hash from the root forward: prize-positioning prose and third-party evaluation scores were redacted from `docs/BUILD-LOG.md` and `docs/PRD.md` across all commits, and 18 commits carrying an AI-assistant author identity were reattributed to the repository owner. The tree at HEAD was unchanged by that rewrite. This line named the pre-rewrite hash until the discrepancy was found, which is exactly the drift this section exists to make checkable.
...
- **Blog — [Seven ways to lie to yourself in code](https://jeremiah-sakuda.github.io/Hodi/blog/seven-ways-to-lie-to-yourself-in-code.html)**. The defect ledger: sixty-six defects across nine classes, the four that recurred, the Antigravity SDK assertion that was verified before it was made, and the four structural guards. Created for the All Things Agentic Hackathon.
...
| `split_revocation_worker` | ✓ verified | scripts/deploy_revocation_worker.sh re-executed 2026-08-25: deployed identity read back from Cloud Run as revocation-propagator-sa, authenticated invocation 200 and anonymous 403, and the worker now serves the production revocation path rather than standing beside it. | 2026-08-25T15:52:00Z |
| `revocation_route_worker_cutover` | ✓ verified | Live 2026-08-25 on hodi-evidence-endpoint-00059-z55: POST /api/v1/revoke executed end to end THROUGH the private worker — HTTP 200, one affected grant, all four derived scopes, a real KMS-ECDSA-P256-SHA256 notice signature. The no-fallback property was proven by its own failure first: while the worker was refusing the front door's OIDC token, /api/v1/revoke returned 503 'Revocation worker unavailable; no effects were initiated' rather than quietly executing the cascade locally. A fail-open would have returned 200 and nobody would have known. Warm round-trip re-measured across 7 runs: median 2389 ms. | 2026-08-25T15:55:00Z |
...
Deployed-path timings (measurement surface: `deployed-over-network`, from [docs/metrics.json](docs/metrics.json), re-measured 2026-08-25 on revision `hodi-evidence-endpoint-00059-z55`): buyer API license path 1675 ms warm average when permitted and 1591 ms when denied; revocation cascade 19874 ms cold / 2563 ms warm average; natural-language license path 3926 ms warm average (each request includes one server-side Gemini call); failure-tolerance drill — a looping worker detected, quarantined and rerouted — 1114 ms server-side average against a 1.0 s supervisor deadline, i.e. detection plus recovery costs about 110 ms on top of the deadline it is waiting on; supervisor deadline in production 5.0 s, derived from an observed p95 of 2939 ms with 1.7× headroom.

**Every one of those figures got slower, and the reason is the architecture, not a regression to fix.** The revocation cascade went ~737 ms → ~1896 ms → ~2652 ms and the permitted licence ~712 ms → ~1675 ms as the four conflict-domain roles became four separately-deployed Cloud Run workloads and the cascade itself moved into the propagator's own service. Both paths check that the artist owns the work, that check reads `works`, and `works` now lives in `hodi-identity` behind the rights-custodian service — so an in-process function call became an authenticated HTTPS hop to a workload the front door cannot bypass. The denial is no longer the fast path either: it pays the same hop before it can refuse, which is the correct order, because you cannot decide a request about a work without first establishing the work.

Roughly 1.9 seconds is what it costs to make the conflict boundary infrastructure instead of application code. Before the split, `docs/deployment_status.json` recorded `conflict_domain_separation: in_process_only` — the policy was real and tested, and one process still held credentials for every domain. It now records `verified`, and the front door is **refused by Google IAM** on all four domain databases while still reading the shared grant log. These are published as regressions rather than absorbed, because a latency number that only ever improves is a number someone is managing.
